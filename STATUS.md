...
## 最新迭代：20关成长乐园（缓存 v42）
- [文档] 奔跑时背对操作者、面向赛道，通关后转身庆祝。
- [文档] 20关、五种路边场景；速度与机关密度逐级增加，加入沟槽、横木和摆杆。
- [文档] 每关首通自动增加装备或特效，共20级成长，含跳跃拖尾、碎金、背包、羽翼及披风。
- [实测] 电脑端20关通关、失败与存档恢复通过，旧学习数据保留；旧六关跳关存档可接续。
- [待验] 真实华为平板性能、操作手感和源远的重玩兴趣。
- 详见 docs/playground-20-upgrade.md；单元验证：node tests/arcade.test.cjs。上一轮形象修复见 docs/reward-first-upgrade.md。
