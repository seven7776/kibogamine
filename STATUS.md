...
## 最新迭代：课本阅读恢复（缓存 v44）
- [文档] 本课照片直接展示；整册本机图库支持翻页、选页、放大及手动设置课文首页。
- [实测] 图片顺序、旧版页码隔离、离线阅读、横竖屏布局及学习数据保留通过。
- [待验] 用户确认语文第一课《桂花雨》，现存532张旧版图片不能作为新版整册填充。需要实际新版PDF或扫描图；真华为未测。
- 详见 docs/textbook-reader-v44.md；全量教材填充尚未完成。

## 上一轮：全 App 流程与操作优化（缓存 v43）
- [文档] 首页学科直达、继续上次学习、当天校内与课外安排；学习/打卡新增本机练习记录与附件、历史回看和附件备份。
- [文档] 课外班可改星期并校验时间冲突；打卡连续天数、学习日期口径、无效课文路由与导入校验修正；检查更新保留离线缓存。
- [文档] 乐园新增倒退、停止、前进、加速；倒退不重复得星。成长装备可预览正背面及实际特效。
- [实测] 单元测试、20关自动通关、全 App 主要页面流程、附件照片保存/恢复与新控制模式通过。
- [待验] 真实华为性能和音视频兼容、孩子试玩、实际教材逐页核对及学习核验联动。
- 详见 docs/whole-app-v43.md。测试：node tests/app-tools.test.cjs 与 node tests/arcade.test.cjs。
