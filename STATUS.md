...
## 最新迭代：黑白熊乐园（缓存 v41）
- 先让游戏有吸引力，学习检验暂不加严。
- 修正弧形牙带、红眼口鼻、耳朵裁切、肚皮穿插及动作被重绘打断。
- 六关 3D 换道跳跃挑战、进度星级、特效解锁、暂停与免费重试。
- 电脑端已验证完整通关与失败流程、进度恢复和旧学习数据保留；华为实机手感与孩子评价待验。
- 详见 docs/reward-first-upgrade.md；单元验证：node tests/arcade.test.cjs。
